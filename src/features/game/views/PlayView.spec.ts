...
function makeGameViewModel(gameId = 'g-1'): GameViewModel {
  return {
    gameId,
    puzzleId: 'p-1',
...
describe('PlayView', () => {
  ...
  it('hides the board while paused and restores it on resume', async () => {
    ...
    const wrapper = mount(PlayView, { props: { gameId: 'g-1' } });
    await flushPromises();
    ...
    expect(wrapper.get('[data-testid="paused-banner"]').exists()).toBe(true);
